...
CONF_BASE_PATH="$HOME/.config/anonvpn";
CONF_FILE="anonvpn.ovpn";
configFilePath="$SDIR/$CONF_FILE"
systemConfigFilePath="$CONF_BASE_PATH/$CONF_FILE";
executableFileName="anonvpn";
...
Options:
  ...
  -c | --configuration  Skip adding the script to the user's PATH and just create anonvpn configuration files.
...
if [ $EUID != 0 -a $configOnly != true ]; then
  ...
  echo "Falling back to configuration only mode. You must re-run this script with sudo or manually copy the anonvpn script into your PATH before using the anonvpn command."
  echo
  configOnly="true"
fi
